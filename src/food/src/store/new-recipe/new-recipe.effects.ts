...
import { alertAddSuccess, UserSelector } from '@assistant/common-sdk';
    ...
    add$ = createEffect(() => this.actions$.pipe(
        ...
        mergeMap(([_, recipe, userId]) => {
            ...
            return this.recipesdata.add$(userId, recipe.id, recipe).pipe(
                switchMap(() => [newRecipeAddSuccess({ recipe }), alertAddSuccess(`${recipe.name} was added to your recipes.`, routeFoodRecipe(recipe.id))]),
